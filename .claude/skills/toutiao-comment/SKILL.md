...
description: 头条评论自动化助手。使用此技能当：需要获取头条热点新闻、根据关键字搜索新闻、生成有观点的评论、交互确认后发布评论时。触发场景：用户说"帮我评论头条"、"获取热点新闻"、"搜索新闻"、"发布评论"等。
---

# Toutiao Comment - 头条评论自动化

获取头条热点新闻或根据关键字搜索新闻、生成真人感评论、交互确认后发布。
...
```
┌──────────────────────────────────────────────────────────────────────────────────────────────┐
│ 登录检查 → [热点新闻|关键字搜索] → 选择新闻 → 获取详情 → 生成评论 → 交互确认 → 发布评论 → 验证记录 │
│     ↑                                                                                        │
│     └──────── 未登录时先执行登录流程 ────────────────────────────────────────────────────────────┘
└──────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
| 模式 | 命令/触发 | 用途 |
|------|----------|------|
| **热点新闻** | "获取热点新闻" | 获取当前头条热点榜单 |
| **关键字搜索** | "搜索关于XXX的新闻" | 根据用户关键字搜索相关新闻 |
...
根据用户输入的关键字搜索相关新闻，适用于针对性评论场景。

**运行脚本**：
```bash
# 搜索单个关键字
uv run python .claude/skills/toutiao-comment/scripts/search_news.py "飞驰人生3"

# 搜索多个关键字
uv run python .claude/skills/toutiao-comment/scripts/search_news.py "飞驰人生3" "贺岁档" "春节档电影"
```
...
1. 对每个关键字构建搜索 URL
2. 访问搜索页面，滚动5次加载更多内容
3. 使用多种正则模式提取文章 ID 和标题：
   - 模式1: `"article_id":"xxx","title":"xxx"`
   - 模式2: `"id":"xxx","title":"xxx"`
   - 模式3: `article/xxx` + 附近标题匹配
4. 清理标题中的 HTML 标签和转义字符
5. 过滤已评论的文章
6. 合并多个关键字的搜索结果并去重
7. 保存到 `data/hot_news.json`（复用热点新闻的存储格式）

**使用场景**：
- 用户说"搜索关于 XXX 的新闻"
- 用户说"帮我找关于 XXX 的内容来评论"
- 用户指定特定话题进行评论

**交互示例**：
```
用户: 帮我搜索关于"AI教育"的新闻来评论

AI: 好的，我来搜索"AI教育"相关的新闻...
    [运行 search_news.py 脚本]

    找到以下相关新闻：
    1. AI教育平台获融资... (ID: 7400000000000000001)
    2. 教育部推进AI教学... (ID: 7400000000000000002)
    3. ...
...
**注意事项**：
- 搜索结果依赖头条搜索页面的 HTML 结构
- 如果搜索结果不理想，可以尝试更具体的关键字组合
- 默认返回最多15条未评论的新闻
...
- **search_news.py**: 根据关键字搜索相关新闻
- **publish_comment.py**: 发布评论到指定文章
- **get_article_detail.py**: 获取文章详情内容
