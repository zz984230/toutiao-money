...
**分析**:
- 话题标签格式需要严格匹配活动名称
- 活动标题"上头条 聊热点"中间有空格，话题标签也必须保留空格
- 当前代码没有自动验证话题标签格式
- `generator.py` 生成的内容模板可能存在格式问题

**解决方案**:
1. **新增 `normalize_hashtag()` 函数**：
   - 自动从活动标题生成正确格式的话题标签
   - 规则：`#活动标题#`，保留原标题中的空格和标点
   - 示例："上头条 聊热点" → "#上头条 聊热点#"

2. **话题标签格式验证**：
   - 发布前检查话题标签格式是否与活动标题匹配
   - 如果活动标题包含空格，话题标签也必须包含空格
   - 在发布前自动修正格式错误

3. **更新内容生成提示词**：
   - 在生成发布内容时，明确指定正确的话题标签格式
   - 确保 AI 生成的内容使用正确格式
...
**关键发现**:
- 头条活动系统的话题标签必须严格匹配活动名称
- 空格等格式差异会导致系统无法识别参与
- 需要在代码层面自动处理，避免人为错误

**验证状态**: ✅ 已识别并记录
**技能同步**: 需同步 PATTERNS.md（话题标签格式模式）
