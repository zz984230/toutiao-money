...
- 需要连续多天打卡发布内容
- 每天打卡有递增奖励
- 可能需要在APP中完成（页面显示"在上方区域生成内容"但无输入框）
- 活动页面为运行时页面（`api.toutiaoapi.com/magic/eco/runtime/release/...`）

**识别**：
- 活动名称包含"创作大会"、"对联"、"对下联"
- 奖励说明显示"打卡X天"或"连续发布"
- 页面显示累计打卡天数和递增奖励
- 页面提示"在上方区域生成内容"但当前页面无输入框

**执行流程**：
```bash
# 使用API返回的href直接访问
activity_url = activities[0].href  # 使用 href 字段
# 或通过 participate_from_activity_page() 方法
await client.participate_from_activity_page(activity_id, content)
```
...
**处理策略**：
- 如果页面无输入框且提示"在上方区域生成内容" → 标记为 `skip_requires_app`
- 这类活动可能需要头条APP的特定功能才能完成

**频率**：低优先级，需要APP或长时间打卡
**示例活动**：
- 文学创作大会 (ID: 1855103227862298)
