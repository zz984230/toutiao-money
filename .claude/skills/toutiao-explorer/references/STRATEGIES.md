...
2. **定位并点击活动卡片**
   ...
   # 或通过活动ID查找链接
   playwright-cli eval 'Array.from(document.querySelectorAll("a[href*=\"1856267285707785\"]")).find(a => a.click())'
   ```
...
4. **在活动页面中填写内容**
   - 查找输入框：`[contenteditable="true"]` 或 `textarea`
   - 填写内容并确保话题标签正确
   - 可配图提高吸引力

5. **发布并验证**
   - 点击活动页面的发布按钮（不是微头条发布页）
   - 等待发布成功提示
   - 截图保存证据
...
| 对比项 | 旧方式 (错误) | 新方式 (E001) |
|--------|-----------------|----------------|
| 入口 | 直接访问微头条发布页 | 从创作者中心点击活动卡片 |
| URL | `mp.toutiao.com/profile_v4/weitoutiao/publish` | `mp.toutiao.com/profile_v3_public/public/activity/?...&id={id}` |
| 活动绑定 | 无话题绑定 | 绑定到特定活动 |
| 发布位置 | 普通微头条发布流 | 活动弹窗/页面中发布 |
